...
def __determineInterpolationConstants__(plot=False, valuesPerLine=6):
    ...
    # Load speed of sound file
    data = np.genfromtxt('data/atmosphere/VIRASpeedOfSound.csv', delimiter=';', skip_header=1)

    if util.isDescending(data[:, 0]):
        np.flipud(data)

    if not util.isAscending(data[:, 0]):
        raise ValueError("Speed of sound values should be purely ascending or descending")

    __printSplineCoefficients__('self.tkcSpeedOfSound',
        __determineInterpolationConstants1D__(data[:,0] * 1e3, data[:,1]))
...
def __testSpeedOfSound__():
    ...
    ax.plot(speedOfSound, zDeep / 1e3)
    ax.set_ylabel('Height [km]')
