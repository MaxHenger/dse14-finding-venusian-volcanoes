...
class Gravity:
    ...
    def __init__(self,planet="Venus",method="complex",accuracy=15):
        if planet=="Venus":
    ...
    def __P1__(self,n,x):
        return  1./ ( (-2)**n * np.math.factorial(n) ) \
                * mp.diff( lambda z: (1-z**2)**n ,x,n)
        
    def __P2__(self,n,m,x):
        return  self.__P1__(n,x) if m==0 else \
        ( (2*n+1)/2.* float(np.math.factorial(n-m))/np.math.factorial(n+m) )**0.5 \
        * (-1)**m/(2**n*np.math.factorial(n))*(1-x**2)**(m/2.)\
        *mp.diff( lambda x: (x**2-1)**n ,x,n+m)
    
    def __tinygrav__(self,altitude,longitude,latitude):
        r = altitude+self.R
        g = self.constants.Mu/(r**2) * (1+sum([ (n+1)*(self.R/r)**n \
        * sum([ self.__P2__(n,m,np.cos(np.deg2rad(latitude)))\
        *(self.C[n][m]*np.cos(m*np.deg2rad(longitude)) + self.S[n][m]*np.sin(m*np.deg2rad(longitude)) ) \
        for m in range(2,n+1)]) for n in range(0,len(self.C))  ]) )
        return g
